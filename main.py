...
class NBPApiClient:
    ...
    async def get_rates(self, date: str) -> Optional[dict]:
...
class CurrencyExchangeTool:
    ...
    async def get_exchange_rates(self, days: int) -> List[Optional[dict]]:
        """Pobiera kursy wymiany walut z ostatnich kilku dni."""
        tasks = [self.api_client.get_rates((datetime.today() - timedelta(days=i)).strftime("%Y-%m-%d")) for i in range(1, min(days, self.max_days) + 1)]
...
if __name__ == "__main__":
    try:
        ...
        print(f"Błąd: {e}. Użycie: py main.py <liczba_dni>")
